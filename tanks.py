...
class Bullet():
    ...
    def __init__(self, level, position, direction, damage = 100, speed = allspeed * 5):
...
class Enemy(Tank):
    ...
    def generatePath(self, direction = None, fix_direction = False):
        ...
        if direction == None:
            ...
            if direction in [self.DIR_UP, self.DIR_RIGHT, self.DIR_UPLEFT, self.DIR_UPRIGHT]:
        ...
        for direction in directions:
            if direction == self.DIR_UP and y > 1:
            ...
            elif direction == self.DIR_UPRIGHT and y > 1 and x < 38:
                ...
                if new_pos_rect.collidelist(self.level.obstacle_rects) == -1:
                    ...
                    new_direction = direction
                    break
            elif direction == self.DIR_DOWNRIGHT and y < 38 and x < 38:
                new_pos_rect = self.rect.move(8, 8)
                if new_pos_rect.collidelist(self.level.obstacle_rects) == -1:
                    ...
                    new_direction = direction
                    break
            elif direction == self.DIR_DOWNLEFT and x > 1 and y < 38:
                new_pos_rect = self.rect.move(-8, 8)
                if new_pos_rect.collidelist(self.level.obstacle_rects) == -1:
...
class Game():
    ...
    def reloadPlayers(self):
        ...
        if len(players) == 0:
            ...
            # first player
            x = 12 * self.TILE_SIZE + (self.TILE_SIZE * 2 - 40) / 2
            y = 38 * self.TILE_SIZE + (self.TILE_SIZE * 2 - 40) / 2
    ...
    def draw(self):
        global screen, players, enemies, bullets, bonuses
        ...
        if self.game_over:
            if self.game_over_y > 188:
                ...
                self.game_over_y -= 4
            screen.blit(self.im_game_over, [288, self.game_over_y]) # 176=(416-64)/2            ###tri
    ...
    def drawSidebar(self):
        ...
        x = 640             ###tri
        y = 0
        screen.fill([100, 100, 100], pygame.Rect([640, 0], [64, 640]))              ###tri
...
###tri
        ...
        if pygame.font.get_init():
            ...
            screen.blit(self.font.render("DO AN TRI TUE NHAN TAO", True, pygame.Color('yellow')), [200, 575])
            screen.blit(self.font.render("TRI - THU - THINH", True, pygame.Color('yellow')), [225, 600])


        if self.nr_of_players == 1:
            screen.blit(self.player_image, [230, 400])
        elif self.nr_of_players == 2:
            screen.blit(self.player_image, [230, 425])

        self.writeInBricks("battle", [175, 150])
        self.writeInBricks("city", [225, 250])
###tri
        if put_on_surface:
